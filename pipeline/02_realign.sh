...
do
 ...
 if [[ ! -f $SPLITBAM || $INBAM -nt $SPLITBAM ]]; then
   ...
   gatk SplitNCigarReads --reference $GENOME --input $DDBAM --output $SPLITBAM
   # \
  #  -rf ReassignOneMappingQuality -RMQF 255 -RMQT 60 -U ALLOW_N_CIGAR_READS -fixMisencodedQuals
   if [ -f $SPLITBAM ]; then
