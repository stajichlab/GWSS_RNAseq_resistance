...
write.csv.gz(txi.kallisto$abundance,"reports/kallisto.TPM.csv.gz")
write.csv.gz(txi.kallisto$counts,"reports/kallisto.counts.csv.gz")

oldsamples = samples
newsamples <- samples %>% unite("Loc_Sucep", Location:ImidaclopridStatus, na.rm = TRUE, remove = FALSE)
samples = newsamples

# DEseq2 analyses
Location = factor (samples$Location)
ImidaclopridStatus = factor (samples$ImidaclopridStatus)
sampleTable <- data.frame(suceptibility = ImidaclopridStatus,
...
resultsNames(dds)
#dds <- lfcShrink(dds,coef="condition_Temecula_Susceptible_vs_GeneralBeale_Resistant",type="apeglm")
...
ggplot(pcaData, aes(PC1, PC2, color=Location, shape=ImidaclopridStatus)) +
    geom_point(size=3) +
